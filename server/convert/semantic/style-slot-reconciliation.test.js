...
test("Wave B.5.1 - 1. Reconciliation Audit: exact 66 canonical slots, 0 orphans, 0 undeclared", () => {
  const canonicalKeys = Object.keys(STYLE_SLOTS);
  const definitionKeys = Object.keys(STYLE_SLOT_DEFINITIONS);

  // Exact 66 slots
  assert.equal(canonicalKeys.length, 66, "Canonical slots count must be exactly 66");
  assert.equal(definitionKeys.length, 66, "Defined slots count must be exactly 66");
  ...
  // 0 orphan slots (all 66 are targeted)
  for (const key of canonicalKeys) {
...
test("Wave B.5.1 - 3. Ontology Integrity: 66 slots correctly distributed across 8 semantic roles", () => {
  const allSlots = getAllSlotDefinitions();
  const distribution = {};
  ...
  assert.equal(distribution[SEMANTIC_ROLES.OBJECT], 5, "OBJECT slots count");
  assert.equal(distribution[SEMANTIC_ROLES.EVENT], 11, "EVENT slots count");
  assert.equal(distribution[SEMANTIC_ROLES.STATE], 12, "STATE slots count");
  assert.equal(distribution[SEMANTIC_ROLES.AFFECT], 5, "AFFECT slots count");
  assert.equal(distribution[SEMANTIC_ROLES.ATMOSPHERE], 13, "ATMOSPHERE slots count");
  assert.equal(distribution[SEMANTIC_ROLES.DIALOGUE_ACT], 2, "DIALOGUE_ACT slots count");
  assert.equal(distribution[SEMANTIC_ROLES.NARRATIVE_FUNCTION], 2, "NARRATIVE_FUNCTION slots count");

  // Sum = 16 + 5 + 11 + 12 + 5 + 13 + 2 + 2 = 66
  const total = Object.values(distribution).reduce((a, b) => a + b, 0);
  assert.equal(total, 66, "Total slots distribution must sum to exactly 66");
});
